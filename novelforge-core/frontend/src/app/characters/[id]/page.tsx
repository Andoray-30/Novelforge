...
const CharacterDetailPage: React.FC = () => {
      ...
      <div className="container mx-auto py-6 px-4">
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground mt-2">ID为 {id} 的角色不存在</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  // 计算出场次数
  const appearanceCount = character.example_messages?.length || 0;
          ...
          <Card>
            ...
            <CardContent>
              {character.example_messages && character.example_messages.length > 0 ? (
                <div className="space-y-4">
                  {character.example_messages.map((message: string, index: number) => (
