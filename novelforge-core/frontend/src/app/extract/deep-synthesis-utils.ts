...
export function formatDeepSynthesisBudgetTier(
	tier: DeepSynthesisBudgetTier | string,
): string {
	switch (tier) {
		case "low":
			return "低预算演进（最多 1 轮）";
...
export function formatDeepSynthesisScopeType(
	scope: DeepSynthesisScopeType | string,
): string {
	switch (scope) {
		case "character":
			return "角色";
...
export function formatConvergenceReason(
	...
	if (
		r.includes("quality_target") ||
		r.includes("converged") ||
		r.includes("stable")
	)
		return "资产质量趋于稳定（已收敛）";
	if (
		r.includes("no_actionable") ||
		r.includes("no_assets") ||
		r.includes("empty")
	)
		return "无可消解或提升的资产项";
...
export function groupProposedChangesByAssetType(
	changes: ProposedChange[] | null | undefined,
): Record<DeepSynthesisAssetType, ProposedChange[]> {
...
export function buildDeepSynthesisSelectionState(
	preview: DeepSynthesisResult | null | undefined,
): Record<string, "accepted" | "rejected" | "undecided"> {
	const state: Record<string, "accepted" | "rejected" | "undecided"> = {};
	if (!preview?.preview || !Array.isArray(preview.preview.proposed_changes))
		return state;
...
export function deriveAcceptedRejectedIds(
	selectionState: Record<string, "accepted" | "rejected" | "undecided">,
): { accepted_change_ids: string[]; rejected_change_ids: string[] } {
...
export function sanitizeDeepSynthesisDisplayValue(
...
export function buildDeepSynthesisApplyRequest(params: {
	...
	const { accepted_change_ids: accepted, rejected_change_ids: rejected } =
		deriveAcceptedRejectedIds(params.selectionState);
...
export function formatApplyStatus(
	status: DeepSynthesisApplyResult["status"],
): string {
	switch (status) {
		case "success":
			return "成功";
...
export function formatApplySkipReason(
...
export function canConfirmRealApplyAfterDryRun(
	result: DeepSynthesisApplyResult | null,
): boolean {
	if (!result) return false;
	return result.status === "dry_run" && !hasApplyConflicts(result);
}

export function extractApplyHistoryCounts(
	attempt: ExtractionApplyHistoryItem,
): {
	applied: number;
	skipped: number;
	conflicts: number;
	...
	const acceptanceRate =
		typeof attempt.user_acceptance_rate === "number"
			? `${Math.round(attempt.user_acceptance_rate * 100)}%`
			: "—";
	return { applied, skipped, conflicts, dryRun, acceptanceRate };
}

// === Apply History Filter Types ===

export type ApplyHistoryStatusFilter =
	| "all"
	| "success"
	| "partial"
	| "failed"
	| "dry_run";
export type ApplyHistoryConflictFilter =
	| "all"
	| "has_conflicts"
	| "no_conflicts";
export type ApplyHistoryRunTypeFilter = "all" | "real_apply" | "dry_run";

export interface ApplyHistoryFilters {
	status: ApplyHistoryStatusFilter;
	conflict: ApplyHistoryConflictFilter;
	runType: ApplyHistoryRunTypeFilter;
}

export function matchesApplyHistoryFilters(
	item: ExtractionApplyHistoryItem,
	filters: ApplyHistoryFilters,
): boolean {
	const counts = extractApplyHistoryCounts(item);
	const status = item.status ?? null;

	if (filters.status !== "all") {
		const matchesStatus = status === filters.status;
		const matchesDryRunByCounts = filters.status === "dry_run" && counts.dryRun;
		if (!matchesStatus && !matchesDryRunByCounts) {
			return false;
		}
	}

	if (filters.conflict !== "all") {
		const hasConflicts = counts.conflicts > 0;
		if (filters.conflict === "has_conflicts" && !hasConflicts) return false;
		if (filters.conflict === "no_conflicts" && hasConflicts) return false;
	}

	if (filters.runType !== "all") {
		const isDryRun = counts.dryRun;
		if (filters.runType === "dry_run" && !isDryRun) return false;
		if (filters.runType === "real_apply" && isDryRun) return false;
	}

	return true;
}

export function filterApplyHistory(
	items: ExtractionApplyHistoryItem[],
	filters: ApplyHistoryFilters,
): ExtractionApplyHistoryItem[] {
	return items.filter((item) => matchesApplyHistoryFilters(item, filters));
}
