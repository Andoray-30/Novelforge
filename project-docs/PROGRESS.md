...
### 2026-05-28 Goal 19 续检：provider 与深层入口文案

- 复核：
  - 仓库处于 `main...origin/main` 干净状态后继续 Goal 19。
  - 使用无敏感内容的最小 chat/completions 请求复测当前服务端 provider 配置。
- 结果：
  - provider 仍返回 `401 Unauthorized`，错误含义为令牌状态不可用。
  - 因此仍不能复跑并通过完整“写作 -> save_asset -> editor 候选”链路。
- 已收口：
  - 导入弹窗移除“浏览器端 OpenAI Key”旧文案，改为服务端托管 provider 配置提示。
...
## 2026-06-01 模型网关与 AI 提取价值复盘

- 暂停继续堆新功能，围绕用户提出的“慢模型不等于不可用、fallback 不能冒充 AI、模型选择不能写死”完成一次策略复盘。
- 更新 `project-docs/EXTRACTION_PROVIDER_STRATEGY.md`：
  - 明确当前问题不是单纯 API 地址或模型名问题，而是模型网关波动、任务角色混用、fallback 边界不清造成的系统性风险。
  - 明确 AI 在 NovelForge 中的价值应是语义压缩、关系理解、事件意义判断、世界观归类和写作增益，而不是原文搬运。
  - 明确本地规则负责稳定性，AI 负责理解性，fallback 只能做低置信诊断种子，不能让低质量结果进入 completed / ready。
